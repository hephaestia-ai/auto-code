...
class MarkdownBot(CoreAssistant):
    """
    Auto Code
    ---------
    ...
    """
    def __init__(self):
        super().__init__(assistant_name="Text Refinement for Markdown")
        self.temperature = 0.4 # Leave some room for inference
        self.description = "Writing assistant bot for markdown file refinement and optimization bot. Just write text"
        self.instructions = (
            "No chat response needed, just respond with the markdown text."
            "Focus on instructions in a markdown comment. Then implement them"
            "If none provided, just write comments at the end of doc to improve readability. Keep in comment"
            "Implement using english language and grammar"
            "Have a casual approach and tone"
            "Add formatting where necessary, but keep it simple."
        )
